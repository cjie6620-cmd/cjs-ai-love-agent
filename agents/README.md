...
- `memory_events.py`
  作用：长期记忆事件契约。
  功能：构建 RocketMQ/Outbox 共用的长期记忆提取消息，生成稳定 `task_id`。
- `memory_consumer.py`
  作用：长期记忆消费者。
  功能：消费 RocketMQ 消息，调用结构化模型判断是否沉淀长期记忆并写入 pgvector。
- `memory_outbox_relay.py`
  作用：长期记忆 Outbox 补投任务。
  功能：扫描 MySQL Outbox，把待投递事件补发到 RocketMQ。
- `question_advisor.py`
  作用：问题顾问模块。
...
- 组织记忆召回、知识检索和回复生成。
- 负责对话结束后的持久化与缓存回填。
- 负责登录用户的长期记忆事件写入；匿名访客只保留短期上下文，不写长期记忆。
...
- 长期记忆主路径是“聊天主流程写 Outbox，Docker Relay 投 RocketMQ，Docker Consumer 做提取入库”。
